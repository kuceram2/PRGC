...
int prime_decompose(long num, int primes[]){
    ...
    while (primes[i] <= num){
        ...
        // if (exp > 0){
        //     printf("%i", primes[i]);
        //     if (exp > 1) printf("^%i", exp);
        //     if (num > 1) printf(" x ");
        // }

        if (exp > 1) printf("%i^%i", primes[i], exp);
        if (exp == 1) printf("%i", primes[i]);
        if ((exp > 0) && (num > 1)) printf(" x ");
        i += 1;
    }
    printf("\n");
    return EXIT_SUCCESS;
...
int main(){
    ...
    //for (int i = 0; i < 10;++i) printf("%i, ", primes[i]);
    ...
    if (ret == ERROR_INPUT) return ERROR_INPUT;
    return 0;
}
