...
{
    if ((line == (height - fence - 1)) || line == -1)
    { // if drawing top or bottom line of fence
        if (fence % 2) // if fence size is odd num
            draw_alternating_line(fence, '-', '|'); 
        else
            draw_alternating_line(fence, '|', '-');
    }
    else
...
void draw_house_with_fence(int width, int height, int fence, int new_line)
{
    ...
    for (int i = 0; i < height - 2; ++i)
    {
        printf("X");
        if (i % 2)
            draw_alternating_line(width - 2, 'o', '*'); // for odd lines
        else
            draw_alternating_line(width - 2, '*', 'o');

        // if drawing a fence next to the house, dont use "\n"
        (i > fence_start) ? printf("X") : printf("X\n");
        if (i > fence_start)
            draw_fence(fence, height, i);
    }
...
void build_house(int width, int height, int fence)
{
    draw_roof(width);
    if (fence == 0)
        draw_simple_house(width, height);
    else
    {
